...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False):
    """
    Get 2d position embedding using sin cos fomular
...
def get_2d_sincos_pos_embed_from_grid(embed_dim, grid):
    ...
    Args:
        embed_dim (np.array): Embedding dimension
        grid (Tuple[int, int], optional): The tuple of grid

    Returns:
        pos_embed (int): position embedding 2d: [H * W, D]
    """
...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    """
    Get 1d position embedding from grid

    Args:
        embed_dim (np.array): output dimension for each position
        pos (List): a list of positions to be encoded: size (M,)

    Returns:
        pos_embed (np.ndarray): position embedding 1d: (M, D)
    """
    assert embed_dim % 2 == 0
...
def interpolate_pos_embed(model, checkpoint_model):
    """
    Interpolation position embeddings for high resolution image

    Args:
        model (Class): Defined model
        checkpoint_model : Checkpoint of model
    """
